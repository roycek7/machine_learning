...
def check_convergence(centroid_old, centroids, K):
    distance = [euclidean_distance(centroid_old[i], centroids[i]) for i in range(K)]
...
def get_variance(old_c, current_centroid, K):
    total_sum = 0
    for _ in range(K):
        total_sum += np.linalg.norm(old_c[_] - current_centroid[_])
    print(f'MSE: {total_sum * 100}')
    return total_sum
...
for i in range(20):
    ...
    if n_c:
        if get_variance(old_centroid[i - 1], centroid, K) == 0:
            break
    n_c = True
